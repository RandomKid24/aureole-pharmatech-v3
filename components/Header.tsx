...
import { Menu, X, Phone, Mail, Linkedin, Facebook, Twitter, Globe, ShieldCheck } from 'lucide-react';
      ...
      <div className={`hidden lg:flex bg-aureole-slate text-white py-2.5 transition-all duration-500 ${isScrolled ? 'h-0 opacity-0 overflow-hidden' : 'h-auto opacity-100'}`}>
        <div className="container mx-auto px-16 flex justify-between items-center text-[10px] font-bold uppercase tracking-[0.2em]">
          <div className="flex gap-10 items-center">
            <a href="tel:+918600522240" className="flex items-center gap-3 hover:text-blue-300 transition-colors">
              <Phone className="w-3.5 h-3.5" /> +91 86005 22240
            </a>
            <div className="w-[1px] h-3 bg-blue-800/30"></div>
            <a href="mailto:enquiry@aureolepharmatech.com" className="flex items-center gap-3 hover:text-blue-300 transition-colors">
          ...
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-2 text-blue-400">
              ...
              <span className="animate-pulse">Global Presence: Active</span>
            </div>
            <div className="w-[1px] h-3 bg-blue-800/30"></div>
            <div className="flex gap-4">
              <a href="#" className="hover:text-blue-300 transition-colors"><Linkedin className="w-3.5 h-3.5" /></a>
              <a href="#" className="hover:text-blue-300 transition-colors"><Facebook className="w-3.5 h-3.5" /></a>
              <a href="#" className="hover:text-blue-300 transition-colors"><Twitter className="w-3.5 h-3.5" /></a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`${isScrolled ? 'bg-white/95 shadow-xl py-3' : 'bg-white py-5'} transition-all duration-300 backdrop-blur-md border-b border-slate-100`}>
        <div className="container mx-auto px-8 lg:px-16 flex justify-between items-center">
          <div className="flex items-center gap-5 group cursor-pointer" onClick={() => window.scrollTo({top:0, behavior:'smooth'})}>
            <div className="w-12 h-12 bg-aureole-blue flex items-center justify-center shadow-lg shadow-blue-500/10 transition-transform group-hover:scale-105">
              <ShieldCheck className="text-white w-6 h-6" />
            </div>
            <div className="flex flex-col">
              <span className="font-black text-xl tracking-tighter text-aureole-slate leading-none uppercase">AUREOLE</span>
              <span className="text-[9px] tracking-[0.4em] font-black text-aureole-blue uppercase mt-1">PHARMATECH</span>
            </div>
          </div>
          ...
          <nav className="hidden lg:flex items-center gap-8 xl:gap-10">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-[10px] font-extrabold uppercase tracking-[0.25em] text-aureole-slate hover:text-aureole-blue transition-colors relative group/link"
              >
                {link.name}
        ...
        <div className="absolute top-0 left-0 w-full h-screen bg-white z-[60] p-10 flex flex-col">
          <div className="flex justify-between items-center mb-16">
             <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-aureole-blue flex items-center justify-center">
                  <ShieldCheck className="text-white w-4 h-4" />
                </div>
                <span className="font-black text-aureole-slate tracking-widest uppercase text-xs">Aureole Portal</span>
             </div>
